...
(defrecord my-SetClient [dbclient]
  client/Client
  (open! [this test node]
    (my-SetClient. (client/open! dbclient test node)))
  (setup! [this test]
...
(defn gen-SetClient [conn]
  (my-SetClient. (s/gen-SetClient conn SetClientTranslation)))
...
(defrecord my-CasSetClient [dbclient]
  client/Client
  (open! [this test node]
    (my-CasSetClient. (client/open! dbclient test node)))
  (setup! [this test]
    (when (db/isleader? (:node# (:conn dbclient)))
      (client/setup! dbclient test)) 
    (jepsen/synchronize test))
...
(defn gen-CasSetClient [conn]
  (my-CasSetClient. (s/gen-CasSetClient conn CasSetClientTranslation)))
