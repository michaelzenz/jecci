...
(defrecord my-SequentialClient [dbclient]
  client/Client
  (open! [this test node]
    (my-SequentialClient. (client/open! dbclient test node)))
  (setup! [this test]
...
(defn gen-SequentialClient [table-count tbl-created? conn]
  (my-SequentialClient. (s/gen-SequentialClient table-count tbl-created? conn SequentialClientTranslation)))
